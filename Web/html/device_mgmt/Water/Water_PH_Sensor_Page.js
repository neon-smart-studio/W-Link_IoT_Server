                        ...
                        if(in_json.command!=null){
                            switch(in_json.command){
                                case "Report Water PH Sensor Current Measure":
                                    Update_Water_PH_Sensor_Measure(in_json.device_ID, in_json);
                                    break;
...
{
    GET_Water_PH_Sensor_All_Sensor_Status(map_device_info_list[Number(index)].device_ID, function(rsp_json){
        ...
        show_device_op_settings.push("<h2 class=\"fg-blue\">水pH計狀態:</h2>");
        show_device_op_settings.push("</div>");

        for(var i = 0; i<rsp_json.num_of_water_ph_sensor; i++){
            show_device_op_settings.push("<div class=\"row\">");
            show_device_op_settings.push("<h3 class=\"fg-blue\">水pH計"+(i+1)+"</h3>");
            show_device_op_settings.push("</div>");
            ...
            show_device_op_settings.push("<h2 style=\"margin-top: 5px; text-align: center;\">目前酸鹼度(pH)</h2>");
            ...
            show_device_op_settings.push("<div id=\"dev_"+index+"_current_water_ph_"+i+"\"/>");
            show_device_op_settings.push("</div>");
            show_device_op_settings.push("</div>");
            show_device_op_settings.push("</div>");
            show_device_op_settings.push("</div>");
        ...
        Update_Water_PH_Sensor_Status(map_device_info_list[Number(index)].device_ID, rsp_json);
        Update_Water_PH_Sensor_Measure(map_device_info_list[Number(index)].device_ID, rsp_json);
    });

}

function Update_Water_PH_Sensor_Status(device_ID, json_rsp_dat)
{

}

function Update_Water_PH_Sensor_Measure(device_ID, json_rsp_dat)
{
    ...
    var current_water_ph_whole_part;
    var current_water_ph_comma_part;
    var show_current_temp = [];
    var show_current_water_ph = [];
    var current_temp;
    var current_water_ph;
    
    for(var i = 0; i<json_rsp_dat.num_of_water_ph_sensor; i++){
        
        current_water_ph = json_rsp_dat.sensor_status_list[i].measure_water_ph;
        current_water_ph_whole_part = Math.floor(current_water_ph);
        current_water_ph_comma_part = Math.round((current_water_ph-current_water_ph_whole_part)*100);

        if($("dev_"+dev_index+"_current_water_ph_" + i)!=null){
            show_current_water_ph = [];
            show_current_water_ph.push(current_water_ph_whole_part+"<span>."+current_water_ph_comma_part+"</span><strong style=\"left: 100px\">%</strong>");
            
            $("#dev_"+dev_index+"_current_water_ph_"+i).html("");
            $("#dev_"+dev_index+"_current_water_ph_"+i).html(show_current_water_ph.join(''));
        }
