                        ...
                        if(in_json.command!=null){
                            switch(in_json.command){
                                case "Report TVOC Sensor Current Measure":
                                    Update_TVOC_Sensor_Measure(in_json.device_ID, in_json);
                                    break;
...
{
    GET_TVOC_Sensor_All_Sensor_Status(map_device_info_list[Number(index)].device_ID, function(rsp_json){
        ...
        show_device_op_settings.push("<h2 class=\"fg-indigo\">TVOC揮發性有機物感測器狀態:</h2>");
        show_device_op_settings.push("</div>");

        for(var i = 0; i<rsp_json.num_of_tvoc_sensor; i++){
            show_device_op_settings.push("<div class=\"row\">");
            ...
            show_device_op_settings.push("<h2 style=\"margin-top: 5px; text-align: center;\">TVOC指數</h2>");
            ...
            show_device_op_settings.push("<div class=\"den\" id=\"dev_"+index+"_current_tvoc_show_color_"+i+"\" style=\"background:gray;\">");
            ...
            show_device_op_settings.push("<div id=\"dev_"+index+"_current_tvoc_"+i+"\"/>");
            show_device_op_settings.push("</div>");
            show_device_op_settings.push("</div>");
            show_device_op_settings.push("</div>");
            show_device_op_settings.push("</div>");
            ...
            if(rsp_json.sensor_status_list[i].measure_co2_ppm!=null)
            {
        ...
        Update_TVOC_Sensor_Status(map_device_info_list[Number(index)].device_ID, rsp_json);
        Update_TVOC_Sensor_Measure(map_device_info_list[Number(index)].device_ID, rsp_json);
    });

}

function Update_TVOC_Sensor_Status(device_ID, json_rsp_dat)
{

}

function Update_TVOC_Sensor_Measure(device_ID, json_rsp_dat)
{
    ...
    var color;
    var current_tvoc_whole_part;
    var current_tvoc_comma_part;
    ...
    for(var i = 0; i<json_rsp_dat.num_of_tvoc_sensor; i++){
        current_tvoc = json_rsp_dat.sensor_status_list[i].measure_tvoc;
        current_tvoc_whole_part = Math.floor(current_tvoc);
        current_tvoc_comma_part = Math.round((current_tvoc-current_tvoc_whole_part)*100);

        if($("dev_"+dev_index+"_current_tvoc_" + i)!=null){
            show_current_tvoc = [];
            show_current_tvoc.push(current_tvoc_whole_part+"<span>."+current_tvoc_comma_part+"</span>");
            
            $("#dev_"+dev_index+"_current_tvoc_"+i).html("");
            $("#dev_"+dev_index+"_current_tvoc_"+i).html(show_current_tvoc.join(''));
            
            if(current_tvoc<21){
                color = "red";
            }
            else if(current_tvoc<41){
                color = "orange";
            }
            else if(current_tvoc<71){
                color = "lime";
            }
            else if(current_tvoc<81){
                color = "teal";
            }
            else{
                color = "blue";
            ...
            if($("dev_"+dev_index+"_current_tvoc_show_color_"+i)!=null){
                $("#dev_"+dev_index+"_current_tvoc_show_color_"+i).removeAttr("class");
                $("#dev_"+dev_index+"_current_tvoc_show_color_"+i).addClass('den bg-' + color);
            }
        }
        ...
        if(rsp_json.sensor_status_list[i].measure_co2_ppm!=null)
        {
            ...
            if($("dev_"+dev_index+"_current_co2_ppm_" + i)!=null){
                ...
                else if(current_co2<41){
                    color = "orange";
                }
                else if(current_co2<71){
                    color = "lime";
                }
                else if(current_co2<81){
                    color = "teal";
                }
                else{
                    color = "blue";
