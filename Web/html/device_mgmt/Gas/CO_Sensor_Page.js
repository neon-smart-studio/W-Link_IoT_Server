                        ...
                        if(in_json.command!=null){
                            switch(in_json.command){
                                case "Report CO Sensor Current Measure":
                                    Update_CO_Sensor_Measure(in_json.device_ID, in_json);
                                    break;
...
{
    GET_CO_Sensor_All_Sensor_Status(map_device_info_list[Number(index)].device_ID, function(rsp_json){
        ...
        show_device_op_settings.push("<h2 class=\"fg-indigo\">CO一氧化碳感測器狀態:</h2>");
        show_device_op_settings.push("</div>");

        for(var i = 0; i<rsp_json.num_of_co_sensor; i++){
            show_device_op_settings.push("<div class=\"row\">");
            ...
            show_device_op_settings.push("<h2 style=\"margin-top: 5px; text-align: center;\">CO濃度(ppm)</h2>");
            ...
            show_device_op_settings.push("<div class=\"den\" id=\"dev_"+index+"_current_co_ppm_show_color_"+i+"\" style=\"background:gray;\">");
            ...
            show_device_op_settings.push("<div id=\"dev_"+index+"_current_co_ppm_"+i+"\"/>");
            show_device_op_settings.push("</div>");
            show_device_op_settings.push("</div>");
            show_device_op_settings.push("</div>");
            show_device_op_settings.push("</div>");
        ...
        Update_CO_Sensor_Status(map_device_info_list[Number(index)].device_ID, rsp_json);
        Update_CO_Sensor_Measure(map_device_info_list[Number(index)].device_ID, rsp_json);
    });

}

function Update_CO_Sensor_Status(device_ID, json_rsp_dat)
{

}

function Update_CO_Sensor_Measure(device_ID, json_rsp_dat)
{
    ...
    var color;
    var current_co_whole_part;
    var current_co_comma_part;
    var show_current_co = [];
    var current_co;
    
    for(var i = 0; i<json_rsp_dat.num_of_co_sensor; i++){
        current_co = json_rsp_dat.sensor_status_list[i].measure_co_ppm;
        current_co_whole_part = Math.floor(current_co);
        current_co_comma_part = Math.round((current_co-current_co_whole_part)*100);

        if($("dev_"+dev_index+"_current_co_ppm_" + i)!=null){
            show_current_co = [];
            show_current_co.push(current_co_whole_part+"<span>."+current_co_comma_part+"</span><strong style=\"left: 100px\">ppm</strong>");
            
            $("#dev_"+dev_index+"_current_co_ppm_"+i).html("");
            $("#dev_"+dev_index+"_current_co_ppm_"+i).html(show_current_co.join(''));
            ...
            if($("dev_"+dev_index+"_current_co_ppm_show_color_"+i)!=null){
                $("#dev_"+dev_index+"_current_co_ppm_show_color_"+i).removeAttr("class");
                $("#dev_"+dev_index+"_current_co_ppm_show_color_"+i).addClass('den bg-' + color);
            }
        }
    }
}
