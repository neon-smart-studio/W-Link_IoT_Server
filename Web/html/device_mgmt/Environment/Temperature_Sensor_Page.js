                        ...
                        if(in_json.command!=null){
                            switch(in_json.command){
                                case "Report Temperature Sensor Current Measure":
                                    Update_Temperature_Sensor_Measure(in_json.device_ID, in_json);
                                    break;
...
{
    GET_Temperature_Sensor_All_Sensor_Status(map_device_info_list[Number(index)].device_ID, function(rsp_json){
        ...
        show_device_op_settings.push("<h2 class=\"fg-green\">溫度計狀態:</h2>");
        show_device_op_settings.push("</div>");

        for(var i = 0; i<rsp_json.num_of_temperature_sensor; i++){
            show_device_op_settings.push("<div class=\"row\">");
            show_device_op_settings.push("<h3 class=\"fg-green\">溫度計"+(i+1)+"</h3>");
            show_device_op_settings.push("</div>");
            ...
            show_device_op_settings.push("<h2 style=\"margin-top: 5px; text-align: center;\">溫度</h2>");
        ...
        Update_Temperature_Sensor_Status(map_device_info_list[Number(index)].device_ID, rsp_json);
        Update_Temperature_Sensor_Measure(map_device_info_list[Number(index)].device_ID, rsp_json);
    });

}

function Update_Temperature_Sensor_Status(device_ID, json_rsp_dat)
{

}

function Update_Temperature_Sensor_Measure(device_ID, json_rsp_dat)
{
    ...
    for(var i = 0; i<json_rsp_dat.num_of_temperature_sensor; i++){
