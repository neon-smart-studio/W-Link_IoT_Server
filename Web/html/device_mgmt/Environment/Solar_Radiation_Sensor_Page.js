                        ...
                        if(in_json.command!=null){
                            switch(in_json.command){
                                case "Report Solar Radiation Sensor Current Measure":
                                    Update_Solar_Radiation_Sensor_Measure(in_json.device_ID, in_json);
                                    break;
...
{
    GET_Solar_Radiation_Sensor_All_Sensor_Status(map_device_info_list[Number(index)].device_ID, function(rsp_json){
        ...
        show_device_op_settings.push("<h2 class=\"fg-green\">太陽照度計狀態:</h2>");
        show_device_op_settings.push("</div>");

        for(var i = 0; i<rsp_json.num_of_solar_radiation_sensor; i++){
            show_device_op_settings.push("<div class=\"row\">");
            show_device_op_settings.push("<h3 class=\"fg-green\">太陽照度計"+(i+1)+"</h3>");
            show_device_op_settings.push("</div>");
            ...
            show_device_op_settings.push("<h2 style=\"margin-top: 5px; text-align: center;\">目前照度(W/m2)</h2>");
            ...
            show_device_op_settings.push("<div id=\"dev_"+index+"_current_solar_radiation_"+i+"\"/>");
            show_device_op_settings.push("</div>");
            show_device_op_settings.push("</div>");
            show_device_op_settings.push("</div>");
            show_device_op_settings.push("</div>");
        ...
        Update_Solar_Radiation_Sensor_Status(map_device_info_list[Number(index)].device_ID, rsp_json);
        Update_Solar_Radiation_Sensor_Measure(map_device_info_list[Number(index)].device_ID, rsp_json);
    });

}

function Update_Solar_Radiation_Sensor_Status(device_ID, json_rsp_dat)
{

}

function Update_Solar_Radiation_Sensor_Measure(device_ID, json_rsp_dat)
{
    ...
    var current_solar_radiation_whole_part;
    var current_solar_radiation_comma_part;
    var show_current_temp = [];
    var show_current_solar_radiation = [];
    var current_temp;
    var current_solar_radiation;
    
    for(var i = 0; i<json_rsp_dat.num_of_solar_radiation_sensor; i++){
        if(rsp_json.sensor_status_list[i].measure_temperature!=null)
        {
        ...
        current_solar_radiation = json_rsp_dat.sensor_status_list[i].measure_solar_radiation;
        current_solar_radiation_whole_part = Math.floor(current_solar_radiation);
        current_solar_radiation_comma_part = Math.round((current_solar_radiation-current_solar_radiation_whole_part)*100);

        if($("dev_"+dev_index+"_current_solar_radiation_" + i)!=null){
            show_current_solar_radiation = [];
            show_current_solar_radiation.push(current_solar_radiation_whole_part+"<span>."+current_solar_radiation_comma_part+"</span><strong style=\"left: 100px\">W/m2</strong>");
            
            $("#dev_"+dev_index+"_current_solar_radiation_"+i).html("");
            $("#dev_"+dev_index+"_current_solar_radiation_"+i).html(show_current_solar_radiation.join(''));
        }
    }
}
