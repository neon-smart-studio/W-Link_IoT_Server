                        ...
                        if(in_json.command!=null){
                            switch(in_json.command){
                                case "Report Door Window Sensor Status Change":
                                    Update_Door_Window_Sensor_Status(in_json.device_ID, in_json);
                                    break;
                            }
                        }
                        break;
...
{
    GET_Door_Window_Sensor_All_Sensor_Status(map_device_info_list[Number(index)].device_ID, function(rsp_json){
        ...
        for(var i = 0; i<rsp_json.num_of_door_window_sensor; i++){
            show_device_op_settings.push("<div class=\"row\">");
            ...
            show_device_op_settings.push("<div style=\"font-size: 25px;margin-top: 45px;\" id=\"current_door_window_sensor_onoff_status_"+i+"\"/>");
            show_device_op_settings.push("</div>");
            show_device_op_settings.push("</div>");
        ...
        Update_Door_Window_Sensor_Status(map_device_info_list[Number(index)].device_ID, rsp_json);
    });

}

function Update_Door_Window_Sensor_Status(device_ID, json_rsp_dat)
{
    ...
    for(var i = 0; i<json_rsp_dat.num_of_door_window_sensor; i++){
        if(json_rsp_dat.sensor_status_list[i].on_off)
        {
            $("#current_door_window_sensor_onoff_status_"+i).html("<i class=\"mif-power mif-4x\" aria-hidden=\"true\"></i>");
        }
        else{
            $("#current_door_window_sensor_onoff_status_"+i).html("<i class=\"mif-flash-off mif-4x\" aria-hidden=\"true\"></i>");
        }
    }
}
