                        ...
                        if(in_json.command!=null){
                            switch(in_json.command){
                                case "Report Dimmable Switch Status Change":
                                    Update_Dimmable_Switch_Status(in_json.device_ID, in_json);
                                    break;
                            }
                        }
                        break;
...
{
    GET_Dimmable_Switch_All_Switch_Status(map_device_info_list[Number(index)].device_ID, function(rsp_json){
        ...
        show_device_op_settings.push("<h2 class=\"fg-red\">調節式開關狀態:</h2>");
        show_device_op_settings.push("</div>");

        for(var i = 0; i<rsp_json.num_of_dimmable_switch; i++){
            show_device_op_settings.push("<div class=\"row\">");
            ...
            show_device_op_settings.push("<div style=\"font-size: 25px;margin-top: 45px;\" id=\"current_dimmable_switch_onoff_status_"+i+"\"/>");
            show_device_op_settings.push("<h2 style=\"margin-top: 40px; text-align: center;\">目前段數</h2>");
            show_device_op_settings.push("<div style=\"font-size: 10px;margin-top: 45px;\" id=\"current_dimmable_switch_level_status_"+i+"\"/>");
            show_device_op_settings.push("</div>");
            show_device_op_settings.push("</div>");
        ...
        Update_Dimmable_Switch_Status(map_device_info_list[Number(index)].device_ID, rsp_json);
    });

}

function Update_Dimmable_Switch_Status(device_ID, json_rsp_dat)
{
    ...
    for(var i = 0; i<json_rsp_dat.num_of_dimmable_switch; i++){
        if(json_rsp_dat.switch_status_list[i].on_off)
        {
            $("#current_dimmable_switch_onoff_status_"+i).html("<i class=\"mif-power mif-4x\" aria-hidden=\"true\"></i>");
        }
        else{
            $("#current_dimmable_switch_onoff_status_"+i).html("<i class=\"mif-flash-off mif-4x\" aria-hidden=\"true\"></i>");
        }
        $("#current_dimmable_switch_level_status_"+i).html("<a class=\"fg-white\">"+Math.round(json_rsp_dat.switch_status_list[i].on_off/255*100)+"</a>");
    }
}
