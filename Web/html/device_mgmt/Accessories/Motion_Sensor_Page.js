                        ...
                        if(in_json.command!=null){
                            switch(in_json.command){
                                case "Report Motion Sensor Status Change":
                                    Update_Motion_Sensor_Status(in_json.device_ID, in_json);
                                    break;
                            }
                        }
                        break;
...
{
    GET_Motion_Sensor_All_Sensor_Info(map_device_info_list[Number(index)].device_ID, function(rsp_json){
        ...
        for(var i = 0; i<rsp_json.num_of_motion_sensor; i++){
            show_device_op_settings.push("<div class=\"row\">");
            ...
            show_device_op_settings.push("<div style=\"font-size: 25px;margin-top: 45px;\" id=\"current_motion_sensor_onoff_status_"+i+"\"/>");
            show_device_op_settings.push("</div>");
            show_device_op_settings.push("</div>");
        ...
        Update_Motion_Sensor_Status(map_device_info_list[Number(index)].device_ID, rsp_json);
    });

}

function Update_Motion_Sensor_Status(device_ID, json_rsp_dat)
{
    ...
    for(var i = 0; i<json_rsp_dat.num_of_motion_sensor; i++){
        if(json_rsp_dat.sensor_status_list[i].on_off)
        {
            $("#current_motion_sensor_onoff_status_"+i).html("<i class=\"mif-power mif-4x\" aria-hidden=\"true\"></i>");
        }
        else{
            $("#current_motion_sensor_onoff_status_"+i).html("<i class=\"mif-flash-off mif-4x\" aria-hidden=\"true\"></i>");
        }
    }
}
