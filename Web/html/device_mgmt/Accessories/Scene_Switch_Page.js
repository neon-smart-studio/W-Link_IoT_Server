...
var scene_switch_individual_switch_info_list = null;
                        ...
                        if(in_json.command!=null){
                            switch(in_json.command){
                                case "Report Scene Switch Status Change":
                                    Update_Scene_Switch_Status(in_json.device_ID, in_json);
                                    break;
                            }
                        }
                        break;
...
{
    GET_Scene_Switch_All_Switch_Info(map_device_info_list[Number(index)].device_ID, function(rsp_json){
        ...
        scene_switch_individual_switch_info_list = rsp_json.switch_info_list;
        ...
        for(var i = 0; i<rsp_json.num_of_scene_switch; i++){
            show_device_op_settings.push("<div class=\"row\">");
            ...
            for(var j = 0; j<rsp_json.switch_info_list[i].support_actions.length; j++){
                var tile_id_str = "device_"+map_device_info_list[Number(index)].device_ID+"_scene_switch_"+i+"_action_"+rsp_json.switch_info_list[i].support_actions[j]+"_tile";
                ...
                show_device_op_settings.push(Resolv_Scene_Switch_Action_Icon(rsp_json.switch_info_list[i].support_actions[j]));
                show_device_op_settings.push("<span class=\"branding-bar\" style=\"font-size: 20px\">"+Resolv_Scene_Switch_Action_Description(rsp_json.switch_info_list[i].support_actions[j])+"</span>");
                show_device_op_settings.push("</div>");
            }
...
function Update_Scene_Switch_Status(device_ID, json_rsp_dat)
{
    ...
    for(var i = 0; i<json_rsp_dat.num_of_scene_switch; i++){
        var tile_id_str = "";
        if(scene_switch_individual_switch_info_list!=null)
        {
            var switch_support_actions = scene_switch_individual_switch_info_list[i].support_actions;
            for(var j = 0; j<switch_support_actions.length; j++){
                tile_id_str = "device_"+device_ID+"_scene_switch_"+i+"_action_"+switch_support_actions[j]+"_tile";
                do_UnMark_Action_Tile(tile_id_str, "bg-darkGrayBlue", "bg-grayBlue");
            }
        }
        tile_id_str = "device_"+device_ID+"_scene_switch_"+i+"_action_"+json_rsp_dat.switch_status_list[i].action+"_tile";
