...
function Handle_Get_Schedule_Operation_Toolbar_Info()
{
    var toolbar_list = [];
...
function Handle_Schedule_Source_Tile_Selected_Event(light_ID)
{
}

function Handle_Schedule_Source_Tile_UnSelected_Event(light_ID)
{
}

function Handle_Schedule_Target_Tile_Selected_Event(light_ID)
{
    Lighting_Identify_With_Effect(light_ID, "OK");
}

function Handle_Schedule_Target_Tile_UnSelected_Event(light_ID)
{
    Lighting_Identify_With_Effect(light_ID, "Blink");
}

function Handle_Schedule_WebSocket_POST_Message(in_json)
{
...
function Handle_Schedule_WebSocket_GET_Message(in_json)
{
    return null;
}
...
function Get_Action_Command_Topic()
{
    return "Lighting";
}
...
{
    var command = $('#schedule_'+index_str+'_select_action').val();
    var command_type = "";
    var param = null;
    ...
    switch(command)
    {
        ...
        case "Move To Level":
            command_type = "Level";
            param = {
                target_level: Math.round(($("#schedule_"+index_str+"_target_level_slider").val())/100*255),
                trans_time: 1,
                with_on_off: false
            }
            break;
...
function Print_Action_Settings_Info(add_schedule, index_str, target_topic_type)
{
    var print_settings_info = [];

    var command = $('#schedule_'+index_str+'_select_action').val();

    $('#schedule_'+index_str+'_target_action_settings').html('');
    ...
    switch(command)
    {
        case "Move To Level":
            ...
            print_settings_info.push("<input type=\"range\" name=\"range\" \
                                        id=\"schedule_"+index_str+"_target_level_slider\" min=\"1\" max=\"100\" value=\"0\" \
                                        onchange=\"$('#schedule_"+index_str+"_target_slider_show_level').val(this.value); \
                                            Print_Save_Settings_Btn('"+add_schedule+"','"+index_str+"')\">");
            print_settings_info.push("<output id=\"schedule_"+index_str+"_target_slider_show_level\">0</output>");
            print_settings_info.push("</div>");
            $('#schedule_'+index_str+'_target_action_settings').html(print_settings_info.join(''));
            break;
        case "Move To Color Temperature":
            print_settings_info.push("<h3 class=\"fg-amber\">色溫調整</h3>");
            print_settings_info.push("<div style=\"width: 85%;margin-left: 0px;\">");
            print_settings_info.push("<div id=\"schedule_"+index_str+"_target_temp_slider\"></div>");
            print_settings_info.push("</div>");
            $('#schedule_'+index_str+'_target_action_settings').html(print_settings_info.join(''));
            ...
            var tempSlider = new iro.ColorPicker("#schedule_"+index_str+"_target_temp_slider", {
                width: 600,
        ...
        case "Move To Color XY":
            ...
            print_settings_info.push("<div id=\"schedule_"+index_str+"_target_color_picker\"></div>");
            print_settings_info.push("</div>");
            $('#schedule_'+index_str+'_target_action_settings').html(print_settings_info.join(''));
        
            var colorPicker = new iro.ColorPicker("#schedule_"+index_str+"_target_color_picker", {
                width: 400,
...
function Show_Schedule_Action_Settings(add_schedule, target_address_type, target_topic_type, index_str, print_dst_id)
{
    ...
    show_target_action_settings.push("<select data-role=\"select\" data-filter=\"false\" data-drop-height=\"100px\" \
                                    id=\"schedule_"+index_str+"_select_action\" \
                                    data-on-item-select=\"Print_Save_Settings_Btn('"+add_schedule+"','"+index_str+"')\" \
                                    data-on-change=\"Print_Action_Settings_Info('"+add_schedule+"','"+index_str+"', '"+target_topic_type+"')\">");
    ...
    show_target_action_settings.push("<div class=\"row\" style=\"width:100%;\">");
    show_target_action_settings.push("<div class=\"cell-12\" id=\"schedule_"+index_str+"_target_action_settings\">");
    show_target_action_settings.push("</div>");
    show_target_action_settings.push("</div>");
...
function Handle_Update_Schedule_Action_Settings_Info(target_address_type, target_topic_type, target_address_ID, index_str, info_json)
{
    $('#schedule_'+index_str+'_select_action').val(info_json.command);
    ...
    switch(info_json.command)
    {
        ...
        case "Turn Off":
            break;
        case "Move To Level":
            $("#schedule_"+index_str+"_target_level_slider").val(Math.round(cmd_param.target_level/255*100));
            break;
        case "Move To Color Temperature":
            break;
        case "Move To Color XY":
